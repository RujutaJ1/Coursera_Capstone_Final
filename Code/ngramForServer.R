...
library(tau)
library(plyr)
setwd("F:/Rujuta/Coursera/Capstone/ngram2")
load("allTrigrams_clean.RData")
load("allBigrams_clean.RData")
load("allTokens_clean.RData")
load("allFourgrams_clean.RData")
load("allTrigrams2_clean.RData")
load("allBigrams2_clean.RData")
load("allTokens2_clean.RData")
load("allFourgrams2_clean.RData")

allBigrams <- rbind(allBigrams,allBigrams2)
allTrigrams <- rbind(allTrigrams,allTrigrams2)
allTokens <- rbind(allTokens,allTokens2)
...
save(allTokens2, file="finalTokens.RData")
...
trigrams <- arrange(trigrams, count)
trigrams <- data.table(trigrams)
save(trigrams, file="finaltrigrams.RData")
...
bigrams <- arrange(bigrams, count)
bigrams <- data.table(bigrams)
save(bigrams, file="finalbigrams.RData")
